...
@Client.on_message(command(["tai", f"tai@{bn}"]) & ~filters.edited)
def song(_, message):
    query = " ".join(message.command[1:])
    m = message.reply("🔎 Đang Tìm Kiếm...")
...
@Client.on_message(
    command(["taiv", f"taiv@{bn}", "video", f"video@{bn}"]) & ~filters.edited
)
async def taiv(client, message):
    ydl_opts = {
        "format": "best",
        "keepvideo": True,
        "prefer_ffmpeg": False,
    ...
    try:
        results = YoutubeSearch(query, max_results=1).to_dict()
        link = f"https://viet69.to{results[0]['url_suffix']}"
        title = results[0]["title"][:40]
        thumbnail = results[0]["thumbnails"][0]
        thumb_name = f"{title}.jpg"
        thumb = requests.get(thumbnail, allow_redirects=True)
        ...
        print(e)
    try:
        msg = await message.reply("📥 **tải xuố...**")
        with YoutubeDL(ydl_opts) as ytdl:
            ytdl_data = ytdl.extract_info(link, download=True)
            file_name = ytdl.prepare_filename(ytdl_data)
    except Exception as e:
        return await msg.edit(f"🚫 **lỗi:** {e}")
    preview = wget.download(thumbnail)
    await msg.edit("📤 **tải lên...**")
...
async def lyrics(_, message):
    try:
        ...
        await rep.edit("❌ **Tìm kiếm thất bại.**\n\n» **Vui lòng thử lại sau.**")
