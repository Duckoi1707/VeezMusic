...
def cb_admin_check(func: Callable) -> Callable:
    async def decorator(client, cb):
        ...
        if cb.from_user.id in admemes:
            ...
            await cb.answer("👤 Người Dùng Bị Cấm Sử Dụng!", show_alert=True)
            return
...
async def music_onoff(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not message.chat.id in DISABLED_GROUPS:
            await lel.edit("» ** trình phát đã được bật**")
            return
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"🔴 **Lỗi chờ lũ** 🔴 \n\n**Không Thể Thêm UserBot Vì Quá Nhiều Yêu Cầu.**"
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        ...
        return
    # await cb.message.edit("🔁 **Chế biến...**")
    x = int(x)
    try:
    ...
    try:
        ...
        if (dur / 60) > DURATION_LIMIT:
            await cb.message.edit(
                f"❌ **âm nhạc với thời lượng nhiều hơn** `{DURATION_LIMIT}` **phút, không thể chơi !**"
            )
            return
    except:
        pass
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        await b.send_photo(
            ...
            reply_markup=keyboard,Request by:** {r_by.mention}",
            reply_markup=keyboard,
        )
    else:
        que[chat_id] = []
...
async def ytplay(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    await USER.join_chat(invitelink)
                    await lel.edit(
                        f"✅ **người sử dụng đã được thêm vào nhóm**",
                    )
    ...
    try:
        ...
        await lel.edit(
            f"💡 ** đã bị cấm trong nhóm này !** \n\n**unban @{ASSISTANT_NAME} và thêm lại vào nhóm này theo cách thủ công.**"
        )
        return
